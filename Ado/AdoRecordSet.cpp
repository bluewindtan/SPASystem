...
BOOL CAdoRecordSet::Load(LPCTSTR strFileName)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: Load 发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
}
...
void CAdoRecordSet::Close()
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: 关闭记录集发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);		
	}
}
...
EditModeEnum CAdoRecordSet::GetEditMode()
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: GetEditMode 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return adEditNone;
	} 
	return	adEditNone; 
...
long CAdoRecordSet::GetPageCount()
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: GetPageCount 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return -1;
	}
}
...
BOOL CAdoRecordSet::SetCacheSize(const long &lCacheSize)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: SetCacheSize方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
	return TRUE;
...
BOOL CAdoRecordSet::SetAbsolutePage(int nPage)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: SetAbsolutePage 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
}
...
long CAdoRecordSet::GetAbsolutePage()
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: GetAbsolutePage 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return -1;
	}
}
...
BOOL CAdoRecordSet::SetAbsolutePosition(int nPosition)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: SetAbsolutePosition 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
}
...
long CAdoRecordSet::GetAbsolutePosition()
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: GetAbsolutePosition 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return -1;
	}
}
...
BOOL CAdoRecordSet::AppendChunk(FieldPtr pField, LPVOID lpData, UINT nBytes)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: AppendChunk 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
}
...
{		
	...
	if ((adFldLong & GetFieldAttributes(strFieldName)) && size > 0)
	{
		...
		if (GetChunk(GetField(strFieldName), (LPVOID)lpData))
		{
			...
			_tcsncpy_s((LPTSTR)&bmpHeader, bmfHeaderLen, (LPCTSTR)lpData, bmfHeaderLen);
...
BOOL CAdoRecordSet::Supports(CursorOptionEnum CursorOptions)
{
	...
	{
		CString sErrMsg;
		sErrMsg.Format(_T("Warning: Supports方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(sErrMsg);
		return FALSE;
	}
	return FALSE;

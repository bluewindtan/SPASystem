...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    ULONGLONG dwLen = sf.GetLength();
    HGLOBAL hMem = sf.Detach();
    if (!hMem) 
        return NULL;

    hMem = ::GlobalReAlloc(hMem, (SIZE_T)dwLen, GMEM_MOVEABLE|GMEM_DDESHARE|GMEM_ZEROINIT);
    if (!hMem) 
        return NULL;
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
	...
	sTemp.Format(_T("Page %d"), pInfo->m_nCurPage);
    pDC->DrawText(sTemp,-1,rc, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_VCENTER);

	sTemp = _T("江都市大桥中学北门");
	pDC->DrawText(sTemp,-1,rc, DT_CENTER | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_VCENTER);

	CTime t = CTime::GetCurrentTime();
	//sTemp = t.Format(_T("%c"));
	sTemp = t.Format(_T("%Y-%m-%d %H:%M:%S"));
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	else if( pCell->m_numHhide!=0||pCell->m_numVhide!=0)//扩充左上角到合适的位置
	{  
		/*CombineRange.Set(nRow,nCol,nRow+pCell->m_numHhide,nCol+pCell->m_numVhide);
		GetCellRangeRect(CombineRange,rect);*/
